...
# Veri dosyaları için global değişkenler
...
def train_model():
    global user_product_matrix, knn_model, df_orders, df_order_items, df_products
    ...
    user_product_matrix = orders_order_items.pivot_table(index='customer_id', columns='product_id', aggfunc='size',
                                                         fill_value=0)
...
def get_user_recommendations():
    ...
    for user_id in users:
        ...
        formatted_predictions = [{'category': category, 'product_ids': [random.choice(product_names) for _ in ids]} for
                                 category, ids in grouped_predictions.items()]
